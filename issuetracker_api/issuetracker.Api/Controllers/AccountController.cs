...
public class AccountController : ControllerBase
{
	...
	public async Task<IActionResult> Signup([FromForm] RegisterDto model)
	{
		...
		if (model.Image != null)
		{
			...
			string uniqueImageName = await ProcessUploadImage(model.Image, $"{model.FirstName}--{model.LastName}");

			imageUrl = $"https://{configuration["AWS:BucketName"]}.s3.amazonaws.com/{uniqueImageName}";
		}
		...
		// 	await s3Client.DeleteImage(user.Image);

		// 	return Problem(
		// 		detail: "we can not send you a verification email please try later!!",
		...
		foreach (var manager in managers)
		{
			await notificationMethods.CreateNotification(
				$"{user.FirstName} have signed up check him out.",
				CreatedInType.User,
				Guid.Parse(user.Id),
				manager);
		}
